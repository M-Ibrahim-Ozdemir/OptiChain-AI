...
<p align="center" style="font-size: 16px; line-height: 1.6; max-width: 850px; margin: 0 auto; text-align: justify;">
  <b>🛠️ Veri Tipi Standardizasyonu, Yıldız Şeması (Star Schema) ve İndeks Optimizasyonu:</b><br>
  Python ETL (psycopg2 bulk load) sonrası PostgreSQL'e TEXT formatında aktarılan ham veriler, veri bütünlüğü için dönüştürülmüştür. İç içe CAST protokolleriyle 11 kimlik değişkeni tam sayıya, finansal ve CRM tahmin metrikleri ise <code>DOUBLE PRECISION</code> tipine mühürlenmiştir. İş birimlerinin sorgu ihtiyaçlarını izole etmek adına, 4 Boyut (Dimension) ve 1 Gerçeklik (Fact) tablosundan oluşan <b>Modern Yıldız Şeması (Star Schema)</b> sanal katmanlar (Views) üzerinden inşa edilmiştir. 
  <br><br>
  <code>v_fact_sales</code> tablosu tüm ciro ve lojistik gerçekleri mühürlerken; müşteri 360, ürün, lokasyon ve zaman boyut tabloları <code>DISTINCT ON</code> optimizasyonuyla en güncel transaksiyonları yakalamaktadır. Panellerin milisaniyeler içinde yanıt vermesi adına, kritik sütunlara B-Tree tabanlı 14 performans indeksi çakılmıştır. Son aşamada <code>VACUUM ANALYZE</code> işletilerek sorgu planlayıcısının (Query Planner) arama maliyetleri sıfıra indirilmiştir.
</p>
...
<div style="max-width: 850px; margin: 0 auto; font-size: 14.5px; line-height: 1.6; text-align: left;">
  <p>🧠 <b>Page 7 - Müşteri Zekası ve Hibrit Segmentasyon (RFM & K-Means):</b> Geleneksel RFM metrikleri ile K-Means kümeleme algoritması çapraz matrise alınmıştır. <b>3.745 VIP şampiyon müşterinin</b> izole edildiği yapıda, ciro ve frekansı yüksek olmasına rağmen sistemsel planlama hataları yüzünden en çok gecikmeye uğrayan riskli kitle <i>(New_High_Risk_Late)</i> belirlenmiş ve proaktif CRM aksiyon hattı tetiklenmiştir.</p>
  
  <p>🎯 <b>Page 8 - Customer 360° & Operational Performance (Drill-Through):</b> Riskli veya şampiyon segmentlerin üzerine sağ tıklanarak tetiklenen bu detay analiz katmanı, <b>5K incelemeye alınan müşterinin 245.02K $ değerindeki risk altındaki finansal hacmini</b> listeler. SQL pencere fonksiyonlarından beslenen bu masada, her müşterinin ömür boyu bıraktığı ciro, en çok harcama yaptığı favori kategorisi <i>(Top Category)</i> ve krizdeki spesifik siparişleri tek satırda birleştirilerek mikro-hedefleme gücü sağlanmıştır.</p>
  
  <p>💎 <b>Page 9 - İleri Düzey CRM & CLTV Gelecek Tahmin Paneli (Lifetimes):</b> BG/NBD ile gelecek 3 aylık satın alma olasılıkları, Gamma-Gamma ile işlem başına beklenen kârlar modellenerek <b>Gelecek 6 Ayda 5.29 Milyon $ Net Kâr (CLTV) Projeksiyonu</b> üretilmiştir. Gelecekteki değerine göre bölünen elit <i>'A - Yıldızlar'</i> segmentinin ciro katkısı 2.09 Milyon $ olarak hesaplanmış ve finansal risk altındaki VIP müşteriler için öncelikli lojistik sevkiyat emri kurgulanmıştır.</p>
</div>
...
<div style="max-width: 850px; margin: 0 auto; font-size: 14.5px; line-height: 1.6; text-align: left;">
  <p>⚖️ <b>Page 10 - Karar Bilimi & İstatistiksel Doğrulama Laboratuvarı (A/B Labs):</b> Python safhasında yürütülen 4 büyük hipotez testi veritabanı entegrasyonuyla panele yansıtılmıştır. <b>Kruskal-Wallis analizi, kargo modlarının kârlılık marjı üzerinde anlamlı bir fark yaratmadığını ($p=0.46$)</b> kanıtlarken; lojistik darboğazın fiziksel hızdan değil, sistemsel planlamadan kaynaklandığını ortaya koymuştur. <b>Mann-Whitney U testi, First Class teslimatların fiziksel olarak 2 kat daha hızlı (2.00 gün) ulaştığını</b> belgelerken; <b>Proportions Z-Test ise First Class'ın %95.32 oranında kronik gecikmeye uğradığını ($p=0.00$)</b> doğrulamıştır. Kök nedenin fiziksel yavaşlık değil, sisteme girilen gerçek dışı "1 Gün" vaat parametresi olduğu saptanmış ve taahhüt sürelerinin revize edilmesi gerekliliği tescillenmiştir. Finans-operasyon darboğazı bacağında ise, Debit Kart onay süreçlerindeki provizyon beklemelerinin gecikme riskini anlamlı derecede tetiklediği ($p<0.05$) ciro-gecikme matrisiyle doğrulanmıştır.</p>
</div>
<table align="center" style="margin: 0 auto; border-collapse: collapse; text-align: center;">
  <tr>
    <td style="padding: 10px;">
...
<div style="max-width: 850px; margin: 0 auto; font-size: 14.5px; line-height: 1.6; text-align: left;">
  <p>🤖 <b>Page 11 - ML Gecikme Motoru & Performans Testi:</b> Geliştirilen <i>CatBoost Classifier (V12)</i> modelinin test sonuçları kurumsal KPI paneline entegre edilmiştir. Model; <b>%71.9 Accuracy, %70.9 Precision, %77.8 F1-Score ve %86.2 Recall</b> başarısıyla çalışmaktadır. Eğitim ve test setleri arasındaki F1 farkının (Overfitting Gap) sadece 0.0492 olması modelin ezberlemeden genelleme yaptığını kanıtlarken; en yüksek karar ağırlığının türetilen <code>NEW_CITY_ITEM_QUANTITY_LOAD</code> (Varış şehrindeki anlık paket yığılması) değişkenine ait olduğu saptanmıştır. Operasyonel risk sızıntılarını önlemek adına %86.2'lik Recall gücü korunarak, risklerin siparişler depodayken kontrol altına alınması sağlanmıştır.</p>
  
  <p>🧠 <b>Page 12 - SHAP Analizi & 3 Kritik Risk İzolatörü (XAI):</b> Kara kutu yapay zeka modellerini şeffaflaştıran Açıklanabilir AI (XAI) katmanıdır. Panel üzerinden spesifik bir sipariş seçildiğinde, arka plandaki TreeExplainer motoru milisaniyeler içinde çalışarak ilgili işleme ait gecikme olasılığını çözmektedir. Dinamik tercüme motoru sayesinde siparişi geciktiren <b>1. Kritik Risk, 2. Kritik Risk ve 3. Kritik Risk kök nedenleri (Örn: Hafta başı kamçı etkisi, bölgesel gümrük stresi, mevsimsel yoğunluk)</b> doğrudan yöneticinin ekranına yansıtılarak nokta atışı lojistik müdahale imkanı sunulmuştur.</p>
</div>
<p align="center">
